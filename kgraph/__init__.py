...
class DataIter(Sample):
    ...
    def __init__(self, data: Data, batch_size: int=128, num_threads: int=2,
...
class Predict:
    ...
    def predict_test(self, function, batch_size: int=256):
        results = self.__predict_test(function, self.num_ent, self.num_rel, batch_size)
        return get_result_table(*results, data_name=self.data_name, flags='Test')
    
    def predict_valid(self, function, batch_size: int=256):
        results = self.__predict_valid(function, self.num_ent, self.num_rel, batch_size)
    ...
    def predict_N2N(self, function, batch_size: int=256, show: bool=False):
        ...
        print('1 to 1')
        results['1to1'] += [i for i in self.__predict_test(function, self.num_ent, self.num_rel, batch_size)]
        
        # 1 to n
        self.data.test = data.one2multi
        print('1 to n')
        results['1toN'] += [i for i in self.__predict_test(function, self.num_ent, self.num_rel, batch_size)]
        
        # n to 1
        self.data.test = data.multi2one
        print('n to 1')
        results['Nto1'] += [i for i in self.__predict_test(function, self.num_ent, self.num_rel, batch_size)]
        
        # n to n
        self.data.test = data.multi2multi
        print('n to n')
        results['NtoN'] += [i for i in self.__predict_test(function, self.num_ent, self.num_rel, batch_size)]
    ...
    def calculate_classification_accuracy(self, function, batch_size=1000, threshold=None):
        ...
        accuracy, threshold = run_triple_classification(tmp_function, self.num_ent, batch_size, threshold)
