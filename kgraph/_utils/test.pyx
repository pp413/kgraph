...
# cdef void print_array(float[::1] array):
#     cdef int i
#     printf('[')
...
cdef int global_num = 0

cdef struct IndexElement:
    int i
    float v

cdef bint _compare(IndexElement a, IndexElement b) nogil:
    return a.v >= b.v

cdef float array_min(float[::1] array):
    cdef float tmp
    cdef int i
    tmp = array[0]
...
cdef float array_max(float[::1] array):
    cdef float tmp
    cdef int i
    tmp = array[0]
    for i in range(1, array.shape[0]):
...
cdef float ind_array_min(IndexElement *array, int num):
    cdef float tmp
    cdef int i
    tmp = array[0].v
...
cdef float ind_array_max(IndexElement *array, int num):
    cdef float tmp
    cdef int i
    tmp = array[0].v
    for i in range(1, num):
...
cdef void _argsort(int* index, float[::1] array):
    global global_num
    cdef:
        int _i
        int length = array.shape[0]
        Pool imem = Pool()
        IndexElement* index_array = <IndexElement*>imem.alloc(length, sizeof(IndexElement))
        #IndexElement* index_array = <IndexElement*>malloc(length * sizeof(IndexElement))
    
    global_num += 1
    printf("length, %d, shape, %d, global_num, %d\n", length, array.shape[0], global_num)
    printf("++======+++++++++++++\n")
    printf("min, %f, max, %f\n", array_min(array), array_max(array))
    for _i in range(length):
        index_array[_i].i = _i
        index_array[_i].v = array[_i]
        if global_num > 35 and _i > 14500 or _i < 10:
            printf("i, %d, index_array_ptr[i].i, %d, index_array_ptr[i].v, %f\n", _i, index_array[_i].i, index_array[_i].v)
    
    printf("++######+++++++++++++\n")
    printf("min, %f, max, %f\n", ind_array_min(index_array, length), ind_array_max(index_array, length))

    sort(&index_array[0], &index_array[length-1], _compare)

    for _i in range(length):
        if global_num > 35 and _i > 14500 or _i < 50:
            printf("j, %d, index_array_ptr[i].i, %d, index_array_ptr[i].v, %f\n", _i, index_array[_i].i, index_array[_i].v)
            printf("array, %f\n", array[index_array[_i].i])
        index[_i] = index_array[_i].i
